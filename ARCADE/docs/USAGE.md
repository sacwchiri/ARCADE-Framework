...
and choose `concept`, `design`, `implement`, or `evaluate` in the TUI. The
configured default is `concept`; select a later stage when the project state
and approved work require it.
...
/accept-slice <slice name or number>
/review-code <scope or files>
...
`/review-code` performs a read-only maintainability and compatibility review.
`/accept-slice` runs the Evaluate acceptance workflow and records a separate
report for the requested slice under `docs/validation/acceptance-reports/`.
