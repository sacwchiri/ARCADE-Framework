...
Implement only approved scope. Produce a pre-edit report and a short
implementation plan first, then
...
Before editing code, create a visible todo checklist containing:
...
Keep the checklist current during implementation and include its completed
state in the final handoff. Preserve user-provided commands verbatim when they
become todo items. Mark verification complete only after the check passes;
record unavailable checks as explicit follow-ups.
